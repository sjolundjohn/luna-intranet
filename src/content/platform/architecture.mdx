...
The gateway forwards the request to the actual model (see [Tech stack](/ai/platform/tech-stack) for which model). The response streams back through the same chain in reverse — gateway → proxy → agent → router → Slack — and lands in your DM.
...
The chain is also what makes **switching models trivial**: a new model provider is a one-line change in `luna-ai-proxy`, not a rewrite of every agent. See [Tech stack](/ai/platform/tech-stack) for more on that.

Next: [privacy & security](/ai/platform/privacy) covers how this architecture keeps your conversations safe.
